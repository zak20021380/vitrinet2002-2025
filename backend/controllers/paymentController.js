...
exports.createPaymentRequest = async (req, res) => {
  try {
    const { adOrderId, planSlug, similarPromotionId } = req.body;
    ...
    // پرداخت آنلاین تبلیغ «فروشگاه‌های مشابه»
    if (similarPromotionId) {
      const sellerId = req.user && req.user.sellerId;
      if (!sellerId) {
        return res.status(401).json({ success: false, message: 'احراز هویت نامعتبر است.' });
      }

      const promotion = await SimilarShopPromotion.findOne({
        _id: similarPromotionId,
        sellerId,
        status: { $nin: ['removed'] }
      });
      ...
      if (promotion.paymentStatus === 'verified') {
        return res.status(200).json({
          success: true,
          alreadyPaid: true,
          message: 'پرداخت این درخواست قبلا تایید شده است.'
        });
      }

      const payment = await Payment.create({
        similarPromotionId: promotion._id,
        sellerId,
        amount: promotion.price,
        paymentStatus: 'pending',
        paymentMethod: 'zarinpal',
        type: 'similar_promotion'
      });
...
exports.handlePaymentCallback = async (req, res) => {
  try {
    ...
    if (status === 'success') {
      ...
      if (payment.type === 'similar_promotion' && payment.similarPromotionId) {
        await SimilarShopPromotion.findByIdAndUpdate(payment.similarPromotionId, {
